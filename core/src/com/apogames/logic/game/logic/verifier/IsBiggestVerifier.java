...
public class IsBiggestVerifier extends Verifier {

    public IsBiggestVerifier(Solution solution) {
        super(solution, false, false, false);
        this.setColumn(3);
    }

    public Verifier getCopy() {
        Verifier verifier = new IsBiggestVerifier(super.getSolution());
        verifier.setCheck(this.getCheck());
        return verifier;
    }

    public Verifier getCopyWithSolution(Solution newSolution) {
        Verifier verifier = new IsBiggestVerifier(newSolution);
        verifier.setCheck(this.getCheck());
        return verifier;
    }
    ...
    public boolean check(int first, int second, int third, ArrayList<Solution> possibleSolutions, int index) {
        ...
        if (first > second && first > third) {
            boolean result = getSolution().getFirst() > getSolution().getSecond() && getSolution().getFirst() > getSolution().getThird();
            if (result) {
                this.setCheck("first " + loc.getCommon().get("verifier_isbiggest_check_isbiggest"));
            } else {
                this.setCheck("first " + loc.getCommon().get("verifier_isbiggest_check_isnotbiggest"));
        ...
        if (second > first && second > third) {
            boolean result = getSolution().getSecond() > getSolution().getFirst() && getSolution().getSecond() > getSolution().getThird();
            if (result) {
                this.setCheck("second " + loc.getCommon().get("verifier_isbiggest_check_isbiggest"));
            } else {
                this.setCheck("second " + loc.getCommon().get("verifier_isbiggest_check_isnotbiggest"));
                this.removePossibilities(false, true, false, possibleSolutions, index);
            }
            return result;
        }
        if (third > first && third > second) {
            boolean result = getSolution().getThird() > getSolution().getFirst() && getSolution().getThird() > getSolution().getSecond();
            if (result) {
                this.setCheck("third " + loc.getCommon().get("verifier_isbiggest_check_isbiggest"));
            } else {
                this.setCheck("third " + loc.getCommon().get("verifier_isbiggest_check_isnotbiggest"));
    ...
    public String getName() {
        return "Ist die groesste Zahl auch die groesste";
    }
    ...
    public void renderTextBig(MainPanel mainPanel, int changeX, int changeY) {
        String title = com.apogames.logic.common.Localization.getInstance().getCommon().get("verifier_isbiggest_title");
    ...
    public void renderText(MainPanel mainPanel, int changeX, int changeY) {
        ...
        int columnWidth = 133;
        String[] textKeys = {"verifier_isbiggest_col1_text", "verifier_isbiggest_col2_text", "verifier_isbiggest_col3_text"};
