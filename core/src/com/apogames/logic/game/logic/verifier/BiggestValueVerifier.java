...
public class BiggestValueVerifier extends Verifier {

    private final int value;

    public BiggestValueVerifier(Solution solution, int value) {
    ...
    public Verifier getCopy() {
        Verifier verifier = new BiggestValueVerifier(super.getSolution(), this.value);
        verifier.setCheck(this.getCheck());
        return verifier;
    }

    public Verifier getCopyWithSolution(Solution newSolution) {
        Verifier verifier = new BiggestValueVerifier(newSolution, this.value);
        verifier.setCheck(this.getCheck());
        return verifier;
    }
    ...
    public boolean check(int first, int second, int third, ArrayList<Solution> possibleSolutions, int index) {
        ...
        if (first > this.value || second > this.value || third > this.value) {
            if (getSolution().getFirst() > this.value || getSolution().getSecond() > this.value || getSolution().getThird() > this.value) {
                this.setCheck(loc.getCommon().get("verifier_biggestvalue_check_oneisbigger") + " " + this.value);
            } else {
                this.setCheck(loc.getCommon().get("verifier_biggestvalue_check_noneisbigger") + " " + this.value);
            }
    ...
    public void renderTextBig(MainPanel mainPanel, int changeX, int changeY) {
        String title = Localization.getInstance().getCommon().get("verifier_biggestvalue_title").replace("{0}", String.valueOf(this.value));
    ...
    public void renderText(MainPanel mainPanel, int changeX, int changeY) {
        ...
        String col1Text = loc.getCommon().get("verifier_biggestvalue_col1_text").replace("{0}", String.valueOf(this.value));
        ...
        String col2Text = loc.getCommon().get("verifier_biggestvalue_col2_text").replace("{0}", String.valueOf(this.value));
