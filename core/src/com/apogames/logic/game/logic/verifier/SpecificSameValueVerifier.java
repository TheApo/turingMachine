...
public class SpecificSameValueVerifier extends Verifier {

    private final int value;

    public SpecificSameValueVerifier(Solution solution, boolean isFirst, boolean isSecond, boolean isThird, int value) {
    ...
    public Verifier getCopy() {
        Verifier verifier = new SpecificSameValueVerifier(super.getSolution(), isFirst(), isSecond(), isThird(), this.value);
        verifier.setCheck(this.getCheck());
        return verifier;
    }

    public Verifier getCopyWithSolution(Solution newSolution) {
        Verifier verifier = new SpecificSameValueVerifier(newSolution, isFirst(), isSecond(), isThird(), this.value);
        verifier.setCheck(this.getCheck());
        return verifier;
    }
    ...
    public boolean check(int first, int second, int third, ArrayList<Solution> possibleSolutions, int index) {
        ...
        if (check) {
            if (first == this.value) {
                ...
                result += "first " + loc.getCommon().get("common_is") + " " + loc.getCommon().get("common_not") + " " + this.value;
            }
            if (second == this.value) {
                result += " " + loc.getCommon().get("common_or") + " second = "+this.value;
            } else {
                result += " " + loc.getCommon().get("common_or") + " second " + loc.getCommon().get("common_is") + " " + loc.getCommon().get("common_not") + " " + this.value;
            }
            if (third == this.value) {
                result += " " + loc.getCommon().get("common_or") + " third = "+this.value;
            } else {
                result += " " + loc.getCommon().get("common_or") + " third " + loc.getCommon().get("common_is") + " " + loc.getCommon().get("common_not") + " " + this.value;
            }
        } else {
            if (first == this.value) {
                result += "first " + loc.getCommon().get("common_is") + " " + loc.getCommon().get("common_not") + " " + this.value;
    ...
    public String getName() {
        return "Ist eine spezielle Zahl gleich "+this.value;
    }
    ...
    public void renderTextBig(MainPanel mainPanel, int changeX, int changeY) {
        String title = com.apogames.logic.common.Localization.getInstance().getCommon().get("verifier_specificsamevalue_title").replace("{0}", String.valueOf(this.value));
    ...
    public void renderText(MainPanel mainPanel, int changeX, int changeY) {
        ...
        String col1Text = loc.getCommon().get("verifier_specificsamevalue_col1_text").replace("{0}", String.valueOf(this.value));
        ...
        String col2Text = loc.getCommon().get("verifier_specificsamevalue_col2_text").replace("{0}", String.valueOf(this.value));
        ...
        String col3Text = loc.getCommon().get("verifier_specificsamevalue_col3_text").replace("{0}", String.valueOf(this.value));
